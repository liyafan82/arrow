...
public abstract class SqlPerformancefTestBase {
   ...
   */
  public ValueVector[] createVectors(Field[] fields, boolean fillData, VectorFactory.VectorType vectorType) {
    ValueVector[] ret = new ValueVector[fields.length];
    for (int i = 0; i < fields.length; i++) {
      // create an empty vector
      ret[i] = createVector(fields[i], vectorType);
   ...
   */
  protected ValueVector createVector(Field field, VectorFactory.VectorType vecType) {
    ...
    if (vectorType instanceof ArrowType.Bool) {
      ...
      switch (floatType.getPrecision()) {
        ...
        case DOUBLE:
          return VectorFactory.createFloat8Vector(vecType, name, allocator);
        default:
